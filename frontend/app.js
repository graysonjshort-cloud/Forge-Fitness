const API = window.ForgeNative?.apiBase?.() || localStorage.getItem("forge_api_url") || (
location.protocol==="https:" ? location.origin : `http://${location.hostname}:8000`
);
let authToken = window.__FORGE_BOOT_AUTH_TOKEN || localStorage.getItem("forge_auth_token") || "";
let account = null;
...
const send=(p,o)=>ForgeApi.request(API,()=>authToken,S,p,o);
if(opt.localFirst&&window.ForgeLocalWorkout){
 const clean={...opt};delete clean.localFirst;delete clean.localKind;delete clean.localSessionId;
 return ForgeLocalWorkout.mutate(send,path,clean,{workout_id:w()?.workout_id,local_session_id:opt.localSessionId||ForgeLocalWorkout.sessionRef(session),kind:opt.localKind||"mutation"});
}
return ForgeOffline.request(ForgeApi.request,API,()=>authToken,S,path,opt,{workout_id:w()?.workout_id,session_id:session?.session_id});
}
async function syncLocalWorkoutWrites(){
if(!authToken||!S.online||!window.ForgeLocalWorkout)return null;
const send=(p,o)=>ForgeApi.request(API,()=>authToken,S,p,o);
const r=await ForgeLocalWorkout.sync(send);
const local=await ForgeLocalWorkout.activeSession();
if(local&&session&&String(local.local_session_id)===String(session.local_session_id||session.session_id)&&local.server_session_id){
 session={...session,...local,session_id:Number(local.server_session_id)};
 S.lastSessionId=session.session_id;
}
if(r.status==="synced"&&r.replayed){
 try{plan=await api("/me/plan/current");await ForgeLocalWorkout.cachePlan(plan)}catch{}
}
return r;
}
async function replayOfflineWorkoutWrites(){
if(!authToken||!S.online)return;
const local=await syncLocalWorkoutWrites();
if(local?.replayed)toast(`${local.replayed} local workout change${local.replayed===1?"":"s"} synced`);
if(!ForgeOffline.count())return;
...
if(r.status==="synced"&&r.replayed){toast(`${r.replayed} legacy offline change${r.replayed===1?"":"s"} synced`);try{plan=await api("/me/plan/current");await ForgeLocalWorkout?.cachePlan?.(plan)}catch{}render()}else if(r.status==="blocked")toast("Recover the workout before syncing offline changes");
}
...
await ForgeLocalWorkout?.cacheBootstrap?.({account,profile,plan});
...
if(new URLSearchParams(location.search).get("return_to")==="calendar"&&plan)S.route="calendarsettings";
 ...
 const offline=!navigator.onLine||/offline|reach the server|timed out|fetch/i.test(e?.message||"");
 if(offline&&window.ForgeLocalWorkout){
   const cached=await ForgeLocalWorkout.bootstrapSnapshot();
   if(cached.account&&cached.plan){
     account=cached.account;S.name=account.display_name||"Athlete";
     if(cached.profile)Object.assign(profile,cached.profile);
     plan=cached.plan;
     const local=cached.session;
     if(local&&local.status==="active"){
       session={...local};
       S.wi=Math.max(0,Number(local.workout_index)||0);
       S.ei=Math.max(0,Number(local.exercise_index)||0);
       S.set=Math.max(0,Number(local.set_index)||0);
       S.restRemaining=Math.max(0,Number(local.rest_remaining)||0);
       S.restTotal=Math.max(0,Number(local.rest_total)||0);
       S.restContext=local.rest_context||null;
     }
     S.route="home";return;
   }
 }
 if(/session expired/i.test(e?.message||"")){
   await ForgeDeviceServices?.clearAuthToken?.();localStorage.removeItem("forge_auth_token");authToken="";account=null;plan=null;
 }else{
   S.startupError=e?.message||"Forge could not load";
 }
}
}
...
await ForgeLocalWorkout?.adoptServerSession?.({sessionId:s.session_id,workoutId:s.workout_id,workoutIndex:S.wi,exerciseIndex:S.ei,setIndex:S.set,workoutSnapshot:plan?.workouts?.[S.wi]});session={...(await ForgeLocalWorkout?.activeSession?.()||session)};
...
if((!S.online||navigator.onLine===false)&&ForgeLocalWorkout?.isLocalOnly?.(session))return {status:"local",session_id:session.session_id};
if(S.online&&ForgeLocalWorkout?.isLocalOnly?.(session))await syncLocalWorkoutWrites();
...
ForgeLocalWorkout?.updatePosition?.({exerciseIndex:S.ei,setIndex:S.set,restRemaining:S.restRemaining,restTotal:S.restTotal,restContext:S.restContext});
}
async function persistPosition(){
if(!session)return;
persistWorkoutSnapshot();
try{await api("/me/session/position",{method:"POST",localFirst:true,localKind:"position",body:JSON.stringify({session_id:ForgeLocalWorkout?.sessionRef?.(session)||session.session_id,exercise_index:S.ei,set_index:S.set})})}catch(e){console.warn(e)}
persistWorkoutSnapshot();
}
...
if(session){try{await api("/me/session/rest/start",{method:"POST",localFirst:true,localKind:"rest_start",body:JSON.stringify({session_id:ForgeLocalWorkout?.sessionRef?.(session)||session.session_id,duration_seconds:seconds})})}catch(e){console.warn(e)}}
go("timer");
}
async function clearPersistentRest(){
if(session){try{await api("/me/session/rest/clear",{method:"POST",localFirst:true,localKind:"rest_clear",body:JSON.stringify({session_id:ForgeLocalWorkout?.sessionRef?.(session)||session.session_id})})}catch{}}
...
ForgeCache.invalidate("home");ForgeWorkoutState?.clear?.();const ww=w();
if(!ww)throw Error("Workout unavailable");
const send=(p,o)=>ForgeApi.request(API,()=>authToken,S,p,o);
const started=await ForgeLocalWorkout.start(send,{workoutId:ww.workout_id,workoutIndex:S.wi,workoutSnapshot:ww});
session={...started.session};S.lastSessionId=session.server_session_id||session.session_id;S.sessionStartedAt=Date.now();S.completedWorkoutSummary=null;S.ei=0;S.set=0;S.workoutPRs=[];
await persistPosition();
if(S.online&&!ForgeLocalWorkout.isLocalOnly(session))await reconcileSession({silent:true});
go("workout");
}
...
const sync=((!S.online||navigator.onLine===false)&&ForgeLocalWorkout?.isLocalOnly?.(session))?{status:"local"}:await reconcileSession({silent:true});
...
let payload={request_id:requestId(),session_id:ForgeLocalWorkout?.sessionRef?.(session)||session.session_id,exercise_id:e.exercise_id,completed_sets:1,difficulty:rpe,skipped:false};
if(timed){
stopExerciseTimer();
...
const result=await api("/me/performance",{method:"POST",localFirst:true,localKind:"set",body:JSON.stringify(payload)});
...
await persistPosition();await api("/me/workout/complete",{method:"POST",localFirst:true,localKind:"complete",body:JSON.stringify({session_id:ForgeLocalWorkout?.sessionRef?.(session)||session.session_id,completed:true})});
S.lastSessionId=session.server_session_id||session.session_id;await ForgeLocalWorkout?.markComplete?.();if(S.online){try{plan=await api("/me/plan/current");await ForgeLocalWorkout?.cachePlan?.(plan)}catch{}}session=null;ForgeWorkoutState?.clear?.();S.ei=0;S.set=0;go("complete");if(S.online)loadCompletedWorkoutSummary();
...
if(a==="apply-cardio-swap")await applyCardioSwap();if(a==="back-exercise")go("exercise");if(a==="swap-selected"){if(S.selectedSwap)await applySwap(S.selectedSwap);else toast("Select an exercise first");}if(a==="abandon"){if(session&&confirm("Abandon this workout? Logged sets will remain in history.")){await api("/me/workout/abandon",{method:"POST",localFirst:true,localKind:"abandon",body:JSON.stringify({session_id:ForgeLocalWorkout?.sessionRef?.(session)||session.session_id})});await ForgeLocalWorkout?.clearActive?.();session=null;S.ei=0;S.set=0;S.restRemaining=0;ForgeWorkoutState?.clear?.();if(S.online){try{plan=await api("/me/plan/current");await ForgeLocalWorkout?.cachePlan?.(plan)}catch{}}go("home");toast(S.online?"Workout abandoned":"Workout abandoned locally — change will sync later")}}
...
await ForgeDeviceServices?.clearAuthToken?.();localStorage.removeItem("forge_auth_token");ForgeWorkoutState?.clear?.();authToken="";account=null;plan=null;session=null;stopCalendarPolling();S.moreOpen=false;S.route="welcome";render();toast("Signed out");
}
}
...
if(window.ForgeNative?.isNative && window.ForgeDeviceServices){
ForgeDeviceServices.onAppStateChange(({isActive})=>{
  if(isActive){
    S.online=navigator.onLine;
    if(authToken&&plan){
      reconcileSession({silent:true}).catch(()=>{});
      replayOfflineWorkoutWrites().catch(()=>{});
    }
  }else{
    try{persistWorkoutSnapshot()}catch{}
  }
});
ForgeDeviceServices.onBackButton(()=>{
  const back=document.querySelector("#backBtn");
  if(back&&back.style.visibility!=="hidden"){back.click();return}
  if(S.moreOpen){S.moreOpen=false;render();return}
  if(S.route!=="home"&&authToken){go("home");return}
  ForgeDeviceServices.exitApp().catch(()=>{});
});
}
