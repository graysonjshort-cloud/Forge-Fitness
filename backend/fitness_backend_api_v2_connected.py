...
DB_PATH = Path(os.getenv("FORGE_DB_PATH", str(Path(__file__).with_name("fitness_app_initial_database.sqlite")))).expanduser()
...
FORGE_SERVE_FRONTEND = os.getenv("FORGE_SERVE_FRONTEND", "0").strip().lower() in {"1","true","yes","on"}
_native_origins = ["https://localhost", "http://localhost", "capacitor://localhost"]
_configured_origins = [
    x.strip().rstrip("/") for x in os.getenv(
        "FORGE_ALLOWED_ORIGINS",
        os.getenv("FORGE_APP_URL", "")
    ).replace("|", ",").split(",") if x.strip()
]
FORGE_ALLOWED_ORIGINS = list(dict.fromkeys(_configured_origins + _native_origins + [
    "http://127.0.0.1:5500", "http://localhost:5500"
]))

app.add_middleware(
    CORSMiddleware,
    allow_origins=FORGE_ALLOWED_ORIGINS,
...
def release_info():
    return {
        "app":"Forge Fitness",
        "version":"17.3.0",
        "channel":"native-cloud-backend",
        "feature_freeze":False,
        "runtime":"api-only" if not FORGE_SERVE_FRONTEND else "api-plus-web",
        "database_mode":"supabase-snapshot" if database.SUPABASE_DB_URL else "local-sqlite",
        "schema_policy":"migration-safe",
        "workout_write_policy":"local-first-idempotent-sync",
...
def healthz():
    return {"status":"ok","version":"17.3.0","runtime":"api-only" if not FORGE_SERVE_FRONTEND else "api-plus-web"}
...
def readyz():
    try:
        ...
        return {
            ...
            "version":"17.3.0",
            "database":"ready",
            "persistence":"configured" if database.SUPABASE_DB_URL else "local-only",
            "tables":len(stats) if hasattr(stats,"__len__") else None,
        }
    except Exception as exc:
        raise HTTPException(503, f"Database not ready: {type(exc).__name__}")
...
def me_system_health(authorization: Optional[str]=Header(None)):
    ...
    return {"status":"ok" if critical else "degraded","checks":checks,"persistence":"supabase" if database.SUPABASE_DB_URL else "local-sqlite","version":"16.0.1"}
...
# PWA asset routes were retired from the default native backend in v17.3.
# They are only registered when explicitly running the legacy web frontend.
if FORGE_SERVE_FRONTEND:
...
@app.get("/me/training/adaptive-directives")
def me_adaptive_directives(authorization: Optional[str]=Header(None)):
    user=_current_account(authorization)
    return training_intelligence_v14.adaptive_directives_v5(user["user_id"],DB_PATH)
...
# ---------------------------------------------------------
# Legacy web frontend (opt-in only).
# Native production is API-only; APK assets live on-device.
# ---------------------------------------------------------
if FORGE_SERVE_FRONTEND:
    APP_DIR = Path(__file__).resolve().parent
    app.mount("/", StaticFiles(directory=str(APP_DIR), html=True), name="forge-web-legacy")
