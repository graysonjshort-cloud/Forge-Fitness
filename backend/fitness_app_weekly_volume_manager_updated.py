...
class PlanGenerator:
    ...
    def _trim_to_time(self, exercises, max_minutes):
        exercises = exercises[:]
        while len(exercises) > 1 and self._estimate_minutes(exercises) > max_minutes:
        ...
        for e in reversed(exercises):
            while e.sets > 1 and self._estimate_minutes(exercises) > max_minutes:
                e.sets -= 1
        # A very short session may only fit one exercise. Keep one viable
        # exercise rather than returning an over-time multi-exercise workout.
        while len(exercises) > 1 and self._estimate_minutes(exercises) > max_minutes:
            exercises.pop()
        return exercises
    ...
    def generate_plan(self, profile: UserProfile):
        ...
        if profile.minutes_per_workout < 1:
            raise ValueError("minutes_per_workout must be at least 1")
...
class MuscleVolumeRecord:
    """Explicit weekly set accounting for one muscle."""
    muscle: str
    target_sets: int
    previous_sets: int
    current_sets: int
...
class WeeklyVolumeManager:
    """
    Sits between WeeklyProgramController and PlanGenerator.

    It makes weekly volume an explicit control surface:
    ...
    The manager uses the generated plan to calculate actual current volume,
    compares it with target volume, and caps the next week's change.
    """
    ...
    def __init__(
        ...
        default_allowed_change: int = 2,
        min_sets: int = 0,
        max_sets: int = 20,
        generator: Optional[PlanGenerator] = None,
    ):
        self.generator = generator or PlanGenerator()
        self.targets = dict(targets or self.DEFAULT_TARGETS)
        self.default_allowed_change = max(0, int(default_allowed_change))
        self.min_sets = max(0, int(min_sets))
        self.max_sets = max(self.min_sets, int(max_sets))
    ...
    def set_target(self, muscle: str, target_sets: int) -> None:
        muscle = self._canonical_muscle(muscle)
        self.targets[muscle] = max(self.min_sets, min(self.max_sets, int(target_sets)))

    def _empty_volume(self) -> dict[str, int]:
        return {muscle: 0 for muscle in self.targets}

    def calculate_current_sets(self, plan: dict) -> dict[str, int]:
        """Count programmed sets by primary muscle across the entire week."""
        volume = self._empty_volume()

        for workout in plan.get("workouts", []):
            for exercise in workout.get("exercises", []):
                muscle = self._canonical_muscle(exercise.get("primary_muscle", ""))
                sets = int(exercise.get("sets", 0))
                volume.setdefault(muscle, 0)
                volume[muscle] += max(0, sets)

        return volume

    def _allowed_change_for(
        self,
        muscle: str,
        state: Optional[WeeklyProgramState],
    ) -> int:
        allowed = self.default_allowed_change

        # Higher fatigue tightens the volume-change ceiling.
        if state:
            if state.fatigue_score >= 8:
                allowed = 0
            elif state.fatigue_score >= 6:
                allowed = min(allowed, 1)

            if state.consecutive_hard_weeks >= 2:
                allowed = min(allowed, 1)
    ...
    def build_volume_table(
        self,
        current_plan: dict,
        previous_sets: Optional[dict[str, int]] = None,
        state: Optional[WeeklyProgramState] = None,
    ) -> list[MuscleVolumeRecord]:
        current = self.calculate_current_sets(current_plan)
        previous = previous_sets or {}

        muscles = set(self.targets) | set(previous) | set(current)
        records = []

        for muscle in sorted(muscles):
            target = int(self.targets.get(muscle, 0))
            previous_value = int(previous.get(muscle, 0))
            current_value = int(current.get(muscle, 0))
            allowed = self._allowed_change_for(muscle, state)

            # Move toward target, but never beyond the explicit weekly cap.
            desired_change = target - current_value
            if desired_change > 0:
                next_sets = current_value + min(desired_change, allowed)
            elif desired_change < 0:
                next_sets = current_value - min(abs(desired_change), allowed)
            else:
                next_sets = current_value

            next_sets = max(self.min_sets, min(self.max_sets, next_sets))

            records.append(
                MuscleVolumeRecord(
                    muscle=muscle,
                    target_sets=target,
                    previous_sets=previous_value,
                    current_sets=current_value,
                    allowed_change=allowed,
                    next_week_sets=next_sets,
                )
            )
    ...
    @staticmethod
    def _allocate_integer_sets(total: int, weights: list[float], minimums: Optional[list[int]] = None) -> list[int]:
        """Allocate an integer weekly set total proportionally using largest remainders."""
        n = len(weights)
        if n == 0:
            return []
        total = max(0, int(total))
        minimums = minimums or [0] * n
        mins = [max(0, int(x)) for x in minimums]
        if sum(mins) > total:
            mins = [0] * n
        remaining = total - sum(mins)
        positive = [max(0.0, float(w)) for w in weights]
        weight_sum = sum(positive) or float(n)
        raw = [remaining * w / weight_sum for w in positive]
        floors = [math.floor(x) for x in raw]
        allocation = [m + f for m, f in zip(mins, floors)]
    ...
    def _workout_muscle_rows(self, plan: dict) -> dict[str, list[tuple[dict, int]]]:
        """Return actual workout occurrences for each muscle and their current sets."""
        rows = {}
        for workout in plan.get("workouts", []):
            per = {}
            for exercise in workout.get("exercises", []):
                muscle = self._canonical_muscle(exercise.get("primary_muscle", ""))
                sets = max(0, int(exercise.get("sets", 0)))
                if sets:
                    per[muscle] = per.get(muscle, 0) + sets
            for muscle, sets in per.items():
                rows.setdefault(muscle, []).append((workout, sets))
        return rows

    def distribute_weekly_volume(self, plan: dict, weekly_targets: dict[str, int], preserve_one_set: bool = True) -> dict:
        """Distribute each muscle target across workouts, then across exercises."""
        rows_by_muscle = self._workout_muscle_rows(plan)
        for muscle, rows in rows_by_muscle.items():
            target = max(0, int(weekly_targets.get(muscle, 0)))
            workout_alloc = self._allocate_integer_sets(
                target, [sets for _, sets in rows],
                [1 if preserve_one_set else 0 for _ in rows],
            )
            for (workout, _), assigned in zip(rows, workout_alloc):
                exercises = [e for e in workout.get("exercises", []) if self._canonical_muscle(e.get("primary_muscle", "")) == muscle]
                minimums = [1 if preserve_one_set and assigned >= len(exercises) else 0 for _ in exercises]
                alloc = self._allocate_integer_sets(assigned, [max(0, int(e.get("sets", 0))) for e in exercises], minimums)
    ...
    def apply_next_week_volume(self, plan: dict, volume_table: list[MuscleVolumeRecord]) -> dict:
        """Compatibility wrapper for distributing next-week targets."""
        return self.distribute_weekly_volume(plan, {r.muscle: r.next_week_sets for r in volume_table})

    def manage_next_week(
        self,
        current_plan: dict,
        previous_sets: Optional[dict[str, int]] = None,
        state: Optional[WeeklyProgramState] = None,
    ) -> tuple[dict[str, int], list[MuscleVolumeRecord]]:
        """
        Return explicit next-week set targets and the auditable volume table.
        """
        records = self.build_volume_table(current_plan, previous_sets, state)
        next_sets = {record.muscle: record.next_week_sets for record in records}
        return next_sets, records
...
class WeeklyProgramController:
    ...
    def __init__(
        self,
        generator: Optional[PlanGenerator] = None,
        volume_manager: Optional[WeeklyVolumeManager] = None,
    ):
        self.generator = generator or PlanGenerator()
        self.volume_manager = volume_manager or WeeklyVolumeManager()
    ...
    def generate_next_week(
        ...
        # Generator creates the exercise-level plan first.
        generated_plan = self.generator.generate_plan(adjusted_profile)

        # Volume manager sits between the controller and generator output,
        # explicitly deciding the next week's muscle-set targets.
        current_sets = self.volume_manager.calculate_current_sets(generated_plan)
        next_sets, volume_table = self.volume_manager.manage_next_week(
            generated_plan, previous_sets=current_sets, state=state
        )
        plan = self.volume_manager.apply_next_week_volume(generated_plan, volume_table)

        if recommendation == "recovery":
            recovery_targets = {muscle: math.floor(target * 0.70) for muscle, target in next_sets.items()}
            plan = self.volume_manager.distribute_weekly_volume(plan, recovery_targets)
            next_sets = recovery_targets
            volume_table = [
                MuscleVolumeRecord(r.muscle, r.target_sets, r.previous_sets, r.current_sets, r.allowed_change, recovery_targets.get(r.muscle, r.next_week_sets))
        ...
        plan["weekly_volume"] = {
            "next_week_sets": next_sets,
            "table": [asdict(record) for record in volume_table],
        }
...
def create_weekly_volume_manager(
    targets: Optional[dict[str, int]] = None,
    default_allowed_change: int = 2,
) -> WeeklyVolumeManager:
    """Convenience constructor for app/API integrations."""
    return WeeklyVolumeManager(
        targets=targets,
        default_allowed_change=default_allowed_change,
    )
