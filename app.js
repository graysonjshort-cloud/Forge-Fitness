...
return `<div class=focus-workout-top><button class=focus-back data-nav=workout aria-label="Back to workout">‹</button><div><p class=eyebrow>${esc(w().name)} • EXERCISE ${S.ei+1}/${w().exercises.length}</p><h1>${esc(e.name)}</h1><p class=focus-set>Set ${setNum} of ${e.sets}</p></div><button class=focus-demo data-form-demo="${e.exercise_id}" data-form-return=exercise>Form</button></div>
