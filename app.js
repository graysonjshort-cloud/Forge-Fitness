...
function moreSheet(){
  ...
  return `<div class=more-backdrop data-a=close-more>
    <div class=more-sheet onclick="event.stopPropagation()">
      ...
      <div class=more-version>Forge Fitness v14.59.0</div>
    </div>
  </div>`;
}
function finalPolishSettingsCard(){
  return `<div class="card final-settings-card"><div class=row><div><p class=eyebrow>FORGE APP</p><h3>App & account</h3></div><span class=version-pill>v14.59.0</span></div>
...
const CUSTOM_SPLIT_MUSCLES=["Chest","Back","Shoulders","Biceps","Triceps","Quads","Hamstrings","Glutes","Calves","Core"];
...
function ensureCustomSplit(){
  ...
  profile.custom_split=Array.from({length:days},(_,i)=>{const muscles=Array.isArray(existing[i]?.muscles)&&existing[i].muscles.length?[...new Set(existing[i].muscles.filter(x=>CUSTOM_SPLIT_MUSCLES.includes(x)))]:defaults[i%defaults.length];return {name:customDayName(muscles,i),muscles}});
}
...
function customsplit(){ensureCustomSplit();return `<p class=eyebrow>CUSTOM SPLIT</p><h2>Build each training day</h2><p class=muted>Select the muscle groups Forge should train on each day. Every day needs at least one group.</p><div class=big-spacer></div>${profile.custom_split.map((day,i)=>`<div class=card><div class=row><div><p class=eyebrow>DAY ${i+1}</p><h3>${esc(customDayName(day.muscles,i))}</h3></div><span class=muted>${day.muscles.length} group${day.muscles.length===1?"":"s"}</span></div><div class=chips>${CUSTOM_SPLIT_MUSCLES.map(m=>`<button class="chip ${day.muscles.includes(m)?"selected":""}" data-custom-muscle="${i}:${m}">${m}</button>`).join("")}</div></div><div class=spacer></div>`).join("")}<button class=btn data-a=custom-split-done>Use Custom Split</button>`}
...
const rf=document.querySelector("#registerForm");if(rf)rf.onsubmit=submitRegister;const lf=document.querySelector("#loginForm");if(lf)lf.onsubmit=submitLogin;if(S.route==="progress"&&!S.strengthTrend)loadStrengthTrend();if(S.route==="progress"&&!S.progressHub)loadProgressHub();if(S.route==="plan"&&!S.adaptationPreview)loadAdaptationPreview();if(S.route==="plan"&&!S.recoveryIntelligence)loadRecoveryIntelligence();if(["equipment","equipmentlog"].includes(S.route)&&!S.equipmentLoaded)loadEquipmentLog();if(["exercisedirectory","preferences","preferencepicker"].includes(S.route)&&!S.exerciseDirectory)loadExerciseDirectory();if(S.route==="coach"&&!S.coachLoaded)loadCoach();if(S.route==="coach"&&!S.coachBriefing)loadCoachBriefing();if(S.route==="exercise")loadExerciseRecall();if(S.route==="swapexercise")loadSwapOptions();if(S.route==="cardioswap")loadCardioSwapOptions();if(S.route==="timer")startTimer();if(S.route==="history")loadHistory();if(S.route==="prs")loadPRs();if(S.route==="nutrition"&&!S.nutrition)loadNutrition();
...
async function act(a){
  try{
    ...
    if(a==="workout-builder")go("workoutbuilder");
...
document.querySelector("#moreBtn").onclick=()=>{if(authToken){S.moreOpen=!S.moreOpen;render()}else toast("Forge Fitness v14.59.0")};
