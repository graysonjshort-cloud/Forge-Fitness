...
async function api(path,opt={}){
  ...
  try{return await ForgeApi.request(API,()=>authToken,S,path,opt)}catch(e){
    if(method==="GET"&&navigator.onLine!==false&&!opt.__retried){await new Promise(r=>setTimeout(r,180));return ForgeApi.request(API,()=>authToken,S,path,{...opt,__retried:true})}
    throw e
  }
}
...
async function reconcileSession({silent=false}={}){
  ...
  try{
    ...
    if(!silent&&sync?.stale_sessions_closed)toast("Recovered stale workout session state");
    return sync;
  }catch(e){console.warn("Session reconciliation failed",e);if(!silent)toast("Could not verify active workout");return null}
...
function exerciseProgressionCard(){const p=S.exerciseProgression;if(!p||p.loading)return "";return `<div class="card progression-card"><div class=row><div><p class=eyebrow>PROGRESSION ENGINE 3.0</p><h3>${esc(p.method)}</h3></div><b>${esc(p.status)}</b></div><p>${esc(p.target_rule)}</p><small>${esc(p.reason)}</small><div class=progression-metrics><span><b>${p.sessions_analyzed}</b><small>sessions</small></span><span><b>${p.retention_score??"—"}</b><small>retention</small></span><span><b>${p.plateau_evidence||0}</b><small>plateau evidence</small></span></div>${p.resume_cue?`<div class=spacer></div><small><b>Post-deload:</b> ${esc(p.resume_cue)}</small>`:""}</div>`}
...
return `${workoutFlowCard(e)}<div class=spacer></div>${exerciseProgressionCard()}<div class=spacer></div><div class=exercise-session-top><div><p class=eyebrow>${esc(w().name)}</p><h2>${esc(e.name)}</h2><p class=muted>Set ${S.set+1} of ${e.sets} • Target ${target}</p><div class=set-adjuster><button type=button data-setchange=-1 aria-label="Remove a set">−</button><span><b>${e.sets}</b><small>planned sets</small></span><button type=button data-setchange=1 aria-label="Add a set">+</button></div></div><span class=set-progress-pill>${setPct}%</span></div>
...
<div class=big-spacer></div>${S.liveAdjustment?`<div class="card live-adjustment"><p class=eyebrow>LIVE WORKOUT INTELLIGENCE 2.0</p><h3>${esc(S.liveAdjustment.title)}</h3><p class=muted>${esc(S.liveAdjustment.detail)}</p>${S.sessionIntelligence?.next_set?`<div class=progression-metrics><span><b>${S.sessionIntelligence.next_set.load_adjustment_percent>0?"+":""}${S.sessionIntelligence.next_set.load_adjustment_percent}%</b><small>load cue</small></span><span><b>RPE ≤ ${S.sessionIntelligence.next_set.effort_cap}</b><small>effort cap</small></span><span><b>${S.sessionIntelligence.next_set.rest_seconds}s</b><small>rest</small></span></div>`:""}</div><div class=spacer></div>`:""}
...
if(S.route==="trainingsettings"&&!S.sessionDiagnostics)loadSessionDiagnostics();
...
async function saveSet(){
  ...
  if(S.sessionIntelligence){
    ...
    S.liveAdjustment={title:si.title||"Session adjustment",detail:`${si.why_changed||si.reason||"Forge adjusted the session from your latest set."}${rest}${volume}${si.estimated_remaining_minutes?` • ~${si.estimated_remaining_minutes} min left`:""}`};
